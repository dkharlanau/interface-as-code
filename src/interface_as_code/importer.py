...
def import_csv(csv_path: str|Path, output_dir: str|Path, column_map: dict[str,str]|None=None, system_map: dict[str,str]|None=None)->dict[str,Any]:
    ...
    with Path(csv_path).open(newline="",encoding="utf-8-sig") as handle:
        ...
        for row_no,row in enumerate(reader,2):
            def col(name:str)->str: return column_map.get(name,name)
            def get(name:str,default:str="")->str: return str(row.get(col(name),default) or "").strip()
            ...
            spec={"version":"1.0","interface":{"id":iid,"name":name,"source":{"system":source},"target":{"system":target},"mode":mode,"criticality":get("criticality").lower() if get("criticality").lower() in {"low","medium","high","critical"} else "medium","lifecycle":"proposed"},"ownership":{"business":get("business_owner") or "TODO","technical":get("technical_owner") or "TODO","support":owner},"trigger":{"schedule":get("frequency") or "TODO"} if mode=="batch" else {"event":"TODO"},"contract":contract,"delivery":{"guarantee":"at-least-once" if mode!="sync" else "effectively-once","ordering":"none","idempotency":{"required":True,"key":business_key}},"mapping":{"profile":"imported"},"retry":{"strategy":"manual","replay":"TODO"} if mode!="sync" else {"strategy":"none"},"monitoring":{"owner":owner,"support_route":support,"business_key":business_key,"signals":["technical_failure"]},"reconciliation":{"key":business_key,"frequency":get("reconciliation_frequency") or "daily","source_of_truth":source,"comparison":"TODO"}}
            middleware=get("middleware")
            if middleware:spec["route"]={"middleware":[middleware]}
            for field,val in (("source",src_raw),("target",tgt_raw),("owner",get("owner")),("support_route",get("support_route")),("business_key",get("business_key"))):
                if not val:gaps.append(ImportGap(row_no,iid,field,"Missing in source inventory; explicit TODO placeholder generated."))
            folder=output/iid.lower(); out_path=folder/"interface.yaml"; dump_yaml(spec,out_path); generated.append(str(out_path))
            canonical_fields=("interface_id","name","source","target","protocol","mode","owner","business_owner","technical_owner","support_route","criticality","business_key","middleware","frequency","reconciliation_frequency","message_type")
            lineage[iid]={"row":row_no,"source_file":str(csv_path),"fields":{k:{"source_column":col(k),"source_value":get(k)} for k in canonical_fields if get(k)!=""}}
    report={"source":str(csv_path),"generated":generated,"gaps":[asdict(g) for g in gaps],"lineage":lineage}
